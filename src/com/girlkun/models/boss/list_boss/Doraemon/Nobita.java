...
public class Nobita extends Boss {
 ...
 @Override
    public void reward(Player plKill) {
        ...
        } else if(Util.isTrue(55, 100)){
            Service.gI().dropItemMap(this.zone, new ItemMap(zone, NRs[randomNR], 1, this.location.x, zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id));
        } else {
            if(Util.isTrue(10, 100)){
                Service.gI().dropItemMap(this.zone, new ItemMap(zone, 1053, 1, this.location.x, this.location.y, plKill.id));
            }
            if(Util.isTrue(1, 100)){
                Service.gI().dropItemMap(this.zone, new ItemMap(zone, 2129, 1, this.location.x, this.location.y, plKill.id));
            }
        }
