            ...
            if (pl != null) {
                if (player.isAdmin() || player.nPoint.teleport) {
                    if (!pl.itemTime.isUseAnDanh || player.isAdmin()) {
                        if ((player.isAdmin() || !pl.zone.isFullPlayer()) && !MapService.gI().isMapDoanhTrai(pl.zone.map.mapId)&& !MapService.gI().isMapMaBu(pl.zone.map.mapId)&& !MapService.gI().isMapBanDoKhoBau(pl.zone.map.mapId) && !MapService.gI().isnguhs(pl.zone.map.mapId)&& pl.zone.map.mapId!=172 && pl.zone.map.mapId != 171 && pl.zone.map.mapId !=170) {
